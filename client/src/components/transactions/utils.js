...
export const isInArray = (arrayOfNames, expenseName) => {
  return arrayOfNames.some((e) =>
    expenseName.toLowerCase().includes(e.toLowerCase())
  );
};
...
const filterOnlyForThisMonth = (txn) =>
  txn.filter(
    (item) =>
      moment(item.date, "DD-MM-YYYY").format("M") === moment().format("M")
  );
const filterOnlyForPastMonth = (txn) =>
  txn.filter(
    (item) =>
      moment(item.date, "DD-MM-YYYY").format("M") ===
